...
public class HTMLGameBrowser implements GameListener {
	...
	public void notifyMove(Move move, short[] nags, String preMoveComment, String postMoveComment, int plyNumber,
		...
		m_moves.append(
				"<a name=\"" + m_moveNumber + "\" class=\"" + type + "\" href=\"javascript:go(" + m_moveNumber + ")\">");
		if (m_showMoveNumber) {
	...
	/**
	 * Set the name of the style file. If name is set to null, inline style definition will be used. Default is inline
	 * style.<br>
	...
	/**
	 * Sets the name of an square image. The default names are set according to the following scheme: First letter is the
	 * color of the stone (b, w), second letter the piece (k, q, r, b, n, p) third letter the square color (b, w), extension
	 * is gif. now.gif and nob.gif are used for empty squares.<br>
	 * For instance: wkw.gif determines a white king on a white square, bbb.gif is a black bishop on a black square.
	 *
	 ...
	 * @param outStream   where the HTML will be sent to
	 * @param game        the game to display.
	 * @param contentOnly if true skip header and footer information, use this if you want to produce your own header and
	 *                    footer
	 */
...
//        out.println("function go(num) {window.document.anchors[moveNumber-1].style.background=\"white\"; if (num<0) moveNumber=0; else if (num>" + (m_moveNumber - 1) + ") moveNumber=" + (m_moveNumber - 1) + "; else moveNumber=num; for(i=0;i<64;i++){if ((Math.floor(i/8)%2)==(i%2)) window.document.images[i].src=wimgs[sq[num][i]]; else window.document.images[i].src=bimgs[sq[num][i]];}; window.document.anchors[moveNumber-1].style.background=\"black\";}");
		...
		for (int row = Chess.NUM_OF_ROWS - 1; row >= 0; row--) {
			...
			for (int col = 0; col < Chess.NUM_OF_COLS; col++) {
				int sqi = Chess.coorToSqi(col, row);
				out.print(
						"<td><img src=\"" + getImageForStone(startPos.getStone(sqi), Chess.isWhiteSquare(sqi)) + "\"></td>");
			}
			out.println("</tr>");
		}
		out.println("</table>");
