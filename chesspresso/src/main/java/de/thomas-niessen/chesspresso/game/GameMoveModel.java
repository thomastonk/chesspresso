...
public class GameMoveModel implements Serializable {
	...
	public void setEmptyGameComment(String comment) {
		if (comment != null && !comment.isEmpty()) {
			m_moves = new short[Math.min(32, comment.length() + 4)];
			m_moves[0] = LINE_START;
	 ...
	 *
	 * @return the index of the next move. If the next move does not exist, the index points to a LINE_END, where a next move
	 *         should be inserted.
	 */
	...
	// TN added: The return value is the new index of the current move or -1 if something failed.
